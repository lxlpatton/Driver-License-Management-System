...
<html>
 ...
 %>
  ...
  <form name="form1" id="form1" method="post" action="">
   ...
   <input type="button" name="Submit2" value="新增" onClick="javascript:location.href='newsproclaim_add.jsp';" />
  </form>
	...
	i++;
     %>
      <tr>
        ...
        <td><a href="gg_detail.jsp?id=<%=map.get("id") %>"><%=map.get("biaoti") %></a></td>
        <td><%=map.get("leibie") %></td>
        <td><%=map.get("tianjiaren") %></td>
        <td><%=map.get("dianjilv") %></td>
        <td width="138" align="center"><%=map.get("addtime") %></td>
        <td width="60" align="center"><a href="newsproclaim_modf.jsp?id=<%=map.get("id")%>">修改</a> <a onClick="return confirm('确定要删除这条记录吗?')"  href="newsproclaim_list.jsp?scid=<%=map.get("id")%>">删除</a></td>
      </tr>
       <%} %>
    </table>
