	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("adminlogin"))
		{
			...
			String sql1="";
				if (utype.equals("管理员"))
				{
				 sql1 = "select * from prousers where username='"+username+"' and pwd='"+password+"'  "; 
				 }
				else if (utype.equals("学员"))
				{
				 sql1 = "select * from xueshengxinxi where xueshengxuehao='"+username+"' and mima='"+password+"'  "; 
				 }
				else
				{
					sql1 = "select * from jiaoshixinxi where jiaoshigonghao='"+username+"' and mima='"+password+"' "; 
				}
			...
			if(userlist1.size()==1)
			{
				...
				gor("tiaozhuan.jsp", request, response);
			}else{
				request.setAttribute("error", "");
				go("/login.jsp", request, response);
			}
		...
		if(ac.equals("uppass"))
		{
			...
			HashMap m = dao.getmaps("xueshengxuehao",(String)request.getSession().getAttribute("username"), "xueshengxinxi");
			...
			String sql = "update xueshengxinxi set mima='"+userpass+"' where xueshengxuehao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
		...
		//修改密码
		 ...
		 if(ac.equals("adminuppassjs"))
		{
			String olduserpass = request.getParameter("ymm");
			String userpass = request.getParameter("xmm1");
			String copyuserpass = request.getParameter("xmm2");   
			HashMap m = dao.getmaps("jiaoshigonghao",(String)request.getSession().getAttribute("username"), "jiaoshixinxi");
			if(!(((String)m.get("mima")).equals(olduserpass)))
			{
				request.setAttribute("error", "");
			...
			//String id = (String)user.get("id");
			String sql = "update jiaoshixinxi set mima='"+userpass+"' where jiaoshigonghao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod3.jsp", request, response);
			}
